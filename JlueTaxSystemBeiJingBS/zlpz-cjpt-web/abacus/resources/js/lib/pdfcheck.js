...
function checkPDFPlugin(){
	...
	if((broserVer.browser === 'IE' && broserVer.version >= 7 && broserVer.version <= 11 )
		...
		if( (adobe.plugin == 'Adobe Reader' || adobe.plugin == 'Adobe PDF'|| adobe.plugin == 'Acrobat' || adobe.plugin == 'Adobe Acrobat') && adobe.version > 9
			...
			result = false;
			var os = getOS();
			// 2、adobe版本为：adboe acrobat reader dc、adobe reader 11及以上 、adobe
			...
			if(adobe.plugin == null) {
				layer.confirm('未检测到adobe插件！若已安装，请检查该插件是否被浏览器禁用；否则请重新下载安装如下版本：adboe acrobat reader dc， adobe reader 11及以上，adobe reader xi，adobe reader x，或直接点击下载进行安装。', {
					title: '错误', 
					icon: 5,
					btn: ['直接下载','官网下载'], // 按钮
				...
				layer.confirm('您的adobe插件为：'+ adobe.plugin + adobe.version +'，请重新安装以下版本：adboe acrobat reader dc， adobe reader 11及以上，adobe reader xi，adobe reader x，或直接点击下载进行安装。', {
					title: '错误', 
					icon: 5,
					btn: ['直接下载','官网下载'], // 按钮
					yes: function(index, layero){
