...
viewApp.filter('pzxh',
    function () {
        return function (item) {
            return "<button class=\"layui-btn layui-btn-xs\" onclick='getsbzfmx(\"" + item.sbqdzf + "\",\"" + item.sbfsDm + "\",\"" + item.pzxh + "\",\"" + item.gdslxDm + "\",\"" + item.skssqq + "\",\"" + item.skssqz + "\",\"" + item.sbrq + "\",\"" + item.yzpzzlDm + "\",\"" + item.zsxmDm + "\")'>申报作废</button>";
...
window.onload = function () {
    ...
    if ("3" == gdbsms) {
        ...
        parent.layer.alert('国地部署模式参数错误', {icon: 5});
    }
    ...
    var index = parent.layer.load(2, {time: 10 * 500, shade: 0.1});
    $.ajax({
        type: "post",
        url: "getSsqz.do",
        data: {},
...
function queryBtn() {
    ...
    //扩展结束
    var index = parent.layer.load(2, {offset: top, time: 10 * 500, shade: 0.1});
 ...
 * @param yzpzzlDm
 */
function getsbzfmx(sbqdzf, sbfsDm, pzxh, gdslxDm, skssqq, skssqz, sbrq, yzpzzlDm, zsxmDm) {
