	...
	$.ajax({
        ...
        success: function(data){
    		...
    		if(res.flag=="ok"){
				...
				var contextPath = location.protocol + "//" + window.location.host;
...
function queryTzggList(){
	...
	hasLoadTzgg=true;
	var showCount = 10;
	var url="/zyywn-cjpt-web/tzgg/syTzgg.do";
	var params={};
...
//		czlink="<div class=\"sbtnbox1\"><a href=\"#\" class=\"layui-btn layui-btn-primary layui-btn-xs\"  ly=\""+item.ly+ "\" id= \""+item.id+"\" url =\" " + url+  "\"ysqxxid=\""+ysqxxid+"\" isRead=\""+item.isRead+"\" isCompleted=\""+item.isCompleted+"\" onclick=\"updateDbsxZt(this)\">办理</a></div>";
		czlink="<button class=\"layui-btn layui-btn-xs\" style=\"width: 60px;\" ly=\""+item.ly+ "\" id= \""+item.id+"\" url =\" " + url+  "\"ysqxxid=\""+ysqxxid+"\" isRead=\""+item.isRead+"\" isCompleted=\""+item.isCompleted+"\" ywbm=\"" +item.ywbm+"\" nsrsbh=\""+item.id_nsrsbh +"\" onclick=\"updateDbsxZt(this)\">办理</button>";
...
//				czlink="<div class=\"sbtnbox1\"><a class=\"sbtn sbtn01\" target=\"_blank\" href=\""+item.url+"\">办理</a></div>";
        ...
        var clzt = "";
        if (item.ywbm != null && item.ywbm != "") {
            clzt="未处理";
        }else {
            if(item.isRead=="N"&&item.isCompleted=="N"){
                clzt="未处理";
            }else if(item.isRead=="Y"&&item.isCompleted=="N"){
                clzt="处理中";
            }else if(item.isRead=="Y"&&item.isCompleted=="Y"){
                clzt="已处理";
            }

        }
        var sxsm="<div class=\"sbtnbox1\"><a href=\"javascript:void(0)\" class=\"sbtn sbtn01 clzt01\"  content=\""+item.content+ "\"> "+clzt+" </a></div>";
...
function getSstxTrHtml(num,item){
	...
	if(item.groupMes=="Y"){
		//TODO
		clickfun+="insertMesReadRec('"+item.id+"','"+item.ly+"','"+item.sxzt+"');\"";
	}else{
		clickfun+="updateSSxxzt('"+item.id+"','"+item.ly+"','SSTX','"+item.sxzt+"');\"";
	}
	
	html+="<td style=\"height:24px;\"><a title='"+item.bt+"' onclick='event.preventDefault();' href="+clickfun+">"
...
function updateDbsxZt(obj){
    var ywbm = $(obj).attr("ywbm");
    if (ywbm != null && ywbm != "" && ywbm != "undefined") {
		var wtRequesturl =context_path + "/xxmh/myCenterController/stepIntoWt.do";
        $.ajax({
            type : "post",
            data : {"ywbm":ywbm,"nsrsbh":$(obj).attr("nsrsbh"),"spjlid":$(obj).attr("id")},
            url : wtRequesturl,
            dataType : "json",
            success : function(data){
            	if("00"==data.code){
                    ...
                    layer.alert("业务跳转失败", {title:"提示",icon: 5,offset:150});
            },
            error: function(data){
