...
namespace JlueTaxSystemBeiJing.Code
{
    public class Service :Controller
    {
        ...
        public string getSBCGMessage(int id)
        {
            ...
            gd = repos.getGDTXDate(qc.ywbm);
            string str = "";
            Ywbm bm = (Ywbm)Enum.Parse(typeof(Ywbm), qc.ywbm);
            switch (bm)
            {
                case Ywbm.ybnsrzzs:
                case Ywbm.xgmzzs:
        ...
        public void getsbzfmx(int id, JObject in_jo)
        {
            ...
            Sbzfmx mx;
            Ywbm ywbm = (Ywbm)Enum.Parse(typeof(Ywbm), qc.ywbm.ToLower());
            switch (ywbm)
            {
                case Ywbm.ybnsrzzs:
                    jo.Add("zsxmDm", "10101");
        ...
        public async Task aqsb_getSbqcList(JObject retJobj)
        {
            ...
            foreach (GDTXUserYSBQC item in ysbqclist)
            {
                ...
                Ywbm ywbm = (Ywbm)Enum.Parse(typeof(Ywbm), item.ywbm.ToLower());
                switch (ywbm)
                {
                    case Ywbm.ybnsrzzs:
                        jo.Add("zsxmDm", "10101");
