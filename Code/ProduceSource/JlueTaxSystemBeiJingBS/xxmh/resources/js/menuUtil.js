...
function getUserElemt(result){
    ...
    if(isNSR == "Y"){ //企业用户
        if(hasYhgl=="Y"){//
            $("#userName").attr("href","javascript:window.goIndexUrl('','yhgl','xgmm');").attr("onclick","event.preventDefault();");
        }else{
				...
				if($.inArray(rightmenu.cdmc, qymenu_Array_type1)>-1){
					...
					for(var i=0;i<row;i++){
						...
						for(var j=i*rownum;j<menunum&&j<(i+1)*rownum;j++){
							...
							if(secmenu.hasOwnProperty("realUrl")){
								if(secmenu.hasOwnProperty("sycddkfs")){
									if(secmenu.sycddkfs=="0"){// 本页跳转打开
				            			...
				            			//增加参数，在旧门户页面返回时，自动切换tab页
				            			var realUrl = secmenu.realUrl;
				            			if(realUrl!=null && realUrl!="" && realUrl!=undefined){
				            				...
				            				realUrl+="tabTitle="+$('#topTabul li').eq(rightmenuNum).attr("id");
				            			}
				            			html +="<a href='#' onclick='goIndexUrl(\""+realUrl+"\",\""+secmenu.topMenu+"\",\""+secmenu.subCdtb+"\",\""+secmenu.qxkzsx+"\")'>";
				            		}else if(secmenu.sycddkfs=="2"){// 新页面打开
							...
							html +="<div class=\"imgbox\"><img class=\"lazy\" data-original=\"/xxmh/resources4/tax-images/png_icon/"+secmenu.cdtb+".png\"></div>";
					...
					for(var i=0;i<showrow;i++){
						...
						for(var j=i*rownum; j<menunum && j<(i+1)*rownum; j++){
							...
							if(j%2==0){
								html+="<img src=\"/xxmh/resources4/tax-images/login/gzfw_titico2.png\">"+ secmenu.cdmc +"</span></div>";
							}else{
								html+="<img src=\"/xxmh/resources4/tax-images/login/gzfw_titico1.png\">"+ secmenu.cdmc +"</span></div>";
							}
								...
								if(thirdmenu.hasOwnProperty("realUrl")){
									if(thirdmenu.hasOwnProperty("sycddkfs")){
										if(thirdmenu.sycddkfs=="0"){// 本页跳转打开
					            			...
					            			html +="<li><a href='#' onclick='goIndexUrl(\""+realUrl+"\",\""+thirdmenu.topMenu+"\",\""+thirdmenu.subCdtb+"\",\""+thirdmenu.qxkzsx+"\")'>";
        ...
        success: function (data) {
            if (data.length > 0) {
				...
				for(var i = 0; i < data.length; i++){
	    			var secmenu = data[i];
	    			if(secmenu.hasOwnProperty("realUrl")){
	    				if(secmenu.hasOwnProperty("dkfs") && secmenu.dkfs=="_blank"){
	    					//新窗口打开
