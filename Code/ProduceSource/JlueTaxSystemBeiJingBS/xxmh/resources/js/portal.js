...
function showNsrs(nsrQysqVos,qybdid,isZrrsfJr,yhm,zjhm,zjlx,zjyxq,zrrs){
    ...
    for(var i in nsrQysqVos){
        ...
        if(nsrQysqVos[i].dsnsrsbh!=null&&nsrQysqVos[i].dsnsrsbh!=""&&typeof(nsrQysqVos[i].dsnsrsbh)!="undefined"&&nsrQysqVos[i].gsnsrsbh!=null&&nsrQysqVos[i].gsnsrsbh!=""&&typeof(nsrQysqVos[i].gsnsrsbh)!="undefined"){
            nsrztHtml = '<li class="'+(i==0?'this':'')+'" qybdid='+nsrQysqVos[i].nsrztid+'>'+nsrQysqVos[i].nsrmc+'（'+nsrQysqVos[i].zzNsrmc+'-'+nsrQysqVos[i].yhsfmc+'）  </li>';
        }else if(nsrQysqVos[i].gsnsrsbh!=null&&nsrQysqVos[i].gsnsrsbh!=""&&typeof(nsrQysqVos[i].gsnsrsbh)!="undefined"){
            nsrztHtml = '<li class="'+(i==0?'this':'')+'" qybdid='+nsrQysqVos[i].nsrztid+'>'+nsrQysqVos[i].nsrmc+'（'+nsrQysqVos[i].nsrmc+'-'+nsrQysqVos[i].yhsfmc+'）  （国税）</li>';
        }else if(nsrQysqVos[i].dsnsrsbh!=null&&nsrQysqVos[i].dsnsrsbh!=""&&typeof(nsrQysqVos[i].dsnsrsbh)!="undefined"){
            nsrztHtml = '<li class="'+(i==0?'this':'')+'" qybdid='+nsrQysqVos[i].nsrztid+'>'+nsrQysqVos[i].dsNsrmc+'（'+nsrQysqVos[i].dsNsrmc+'-'+nsrQysqVos[i].yhsfmc+'）  （地税）</li>';
        }
...
function getMenuCallback(data){
        ...
        if(result.flag=="ok"){
            ...
            if(menuHtml!=""){
                ...
                if(isNSR == "Y"){ //企业用户
                    if(hasYhgl=="Y"){//
                        $("#userName").attr("href","javascript:window.goIndexUrl('','yhgl','xgmm');");
                    }else{
            ...
            //获取从文书那边跳转到index.html传过来的参数，跳到指定的菜单
            var yjcd = getURLParameter("yjcd");
            var ejcd = getURLParameter("ejcd");
            var sjcd = getURLParameter("sjcd");
            var from = getURLParameter("from");
            var secGnUrl = getURLParameter("secGnUrl");
            var secCdid = getURLParameter("secCdid");
            var thirdGndm = getURLParameter("thirdGndm");
            if("dzswj_qjss"==from){
                if(yjcd!=null&&yjcd!=""&&ejcd!=null&&ejcd!=""){
                    if(secGnUrl!=null&&secGnUrl!=""){
                        goPage(makeUrl(secGnUrl,secCdid,sjcd,thirdGndm),yjcd,ejcd,'','')
                    }else{
                        if(ejcd=="qtysb"){
                            javascript:goPage('/sbzs-cjpt-web/biz/sbqc/sbqc_qtsb?cdId=233&gnDm=gndm-231&gdslxDm=3','sbjs','qtysb ','','');
                        }else{
                            goPage('',yjcd,ejcd,'','');
    ...
    if(subMenus.length>0){//二级
        for(var i=0;i<subMenus.length;i++){
            ...
            if(secMenu.dkfs=="_blank"){//新窗口打开
                //跳转至北京实名办税身份信息采集页面
                if(secMenu.gndm == "wybs.yhgl.smbssfxxcj"){
                    var bjsmz = goBjsmz("","","","","bjsmzxxUrl");
                    html+="<li "+noerj+"><i class=\"iconfont fsicon-tree-dot\"></i><a class=\"active\" href=\""+bjsmz+"\" target='_blank'>"+secMenu.cdmc+"</a>";
                }else{
                    html+="<li "+noerj+"><i class=\"iconfont fsicon-tree-dot\"></i><a class=\"active\" href=\""+secMenu.gnUrl+"\" target='_blank'>"+secMenu.cdmc+"</a>";
                }
            }else{
            ...
            if(menus.cdtb=="sxbl" &&(secMenu.cdtb !="dzzlgl") ){//事项办理特殊处理
                ...
                for(var k=0;k<thirdMenus.length;k++){
                    ...
                    if(thirdMenu.dkfs=="_blank"){//新窗口打开
						if(thirdMenu.gndm=="gndm-932"){//用户授权管理嵌套北京实名制新窗口打开
							var bjsmz = goBjsmz("","","_blank","","bjsmzUrl");
							html+="<li><a href=\""+bjsmz+"\" target='_blank'>"+thirdMenu.cdmc+"</a>";
						}else{
							html+="<li><a href=\""+thirdMenu.gnUrl+"\" target='_blank'>"+thirdMenu.cdmc+"</a>";
						}
                    }else{
                        ...
                        }else if(mFlag3=="yhsqgl"){//用户授权管理嵌套北京实名制
                            var params3="'"+menus.cdtb+"','"+thirdMenu.cdtb+"','','"+secMenu.qxkzsx+"','bjsmzUrl'";
                            var onclick3= "onclick=\"javascript:goBjsmz("+params3+");\"";
                            html+="<li><a href=\"#none\" mFlag='"+mFlag3+"' pMFlag='"+secMenu.cdtb+"' realurl='' aFlag=\"hasUrl\" "+onclick3+">"+thirdMenu.cdmc+"</a>";
                        }else{
                        ...
                        /*var mFlag3=thirdMenu.cdtb;//getMFlag(thirdMenu);//取得链接标志
                          var params3="'"+makeRealUrl(secMenu.cdid,thirdMenu.cdid,thirdMenu)+"','"+menus.cdtb+"','"+thirdMenu.cdtb+"','','"+secMenu.qxkzsx+"'";
                        var onclick3= "onclick=\"javascript:goPage("+params3+");\"";
                        html+="<li><a href=\"#none\" mFlag='"+mFlag3+"' pMFlag='"+secMenu.cdtb+"' realurl='"
                            +makeRealUrl(thirdMenu.cdid,"",thirdMenu)+"' aFlag=\"hasUrl\" "+onclick3+">"+thirdMenu.cdmc+"</a>";*/
 ...
 * @param pageId
 */
function showGuide(menuId){
    $.ajax({
        url : contPath+"/guide/getNsrGuideInfoList.do?rand="+Math.random(),
        type : "post",
        data : {'menuId' : menuId},
        ...
        success:function(result){
            if(result != null || result != undefined){
                if(result.isShow){
                    $('#guide').html('');
                    $.each(result.guideJsonArray,function(index){
                        var $guide = $(this.guideHtml).addClass('guide-'+menuId+this.xh);//增加class用以区分不同步骤的指引页
                        if(this.xh==1){
                            $guide.show();
                        }
                        $guide.find('h5 .btn-close').show();//显示右上角关闭按钮
                        for(var i=1;i<=result.guideJsonArray.length;i++){//根据配置的指引步骤数量动态添加导航点
                            var iHtml='';
                            iHtml += '<i class="i'+i;
                            if(this.xh==i){
                                iHtml += ' over';
                            }
                            iHtml += '" onclick="javascript:guideSelect(\'.guide-'+menuId+i+'\');"></i>';
                            $guide.find('.link-dot').append(iHtml);
                        }
                        if(this.btnMoreUrl!=null && this.btnMoreUrl!='' && this.btnMoreUrl!=undefined ){//根据配置设置”查看更多“按钮url
                            $guide.find('.guide-btn-box .btn-more').attr("href",this.btnMoreUrl);
                            $guide.find('.guide-btn-box .btn-more').attr("target","_blank");
                        }
                        this.guideHtml = $guide.prop("outerHTML");
                        $('#guide').append(this.guideHtml);
                    });
...
function resizeBack(){
    ...
    $(".guide-select .back-box .left-div").width((docWidth-$(".guide-select .back-box ul").width())/2);
    $(".guide-select .back-box .right-div").width((docWidth-$(".guide-select .back-box ul").width())/2);
}

$(document).ready(function(){
    $(".guide-select .btn-next").live("click", function() {
        $(this).parents(".guide-select").hide().next().show();
    });
    $(".guide-select .btn-previous").live("click", function() {
        $(this).parents(".guide-select").hide().prev().show();
    });
    $(".guide-select .btn-close").live("click", function() {
        $(this).parents(".guide-select").hide();
    });
});
                    ...
                    if (data.code == 0) {
                        if (data.data.length > 0) {//至少保证了下拉框有内容
                           ...
                           $(czgcmcOptions[0]).prop("selected",true);
                            // $("#czgcmc option:eq(0)").prop("selected", 'selected');
 ...
 * 嵌入北京实名制
 */
function goBjsmz(param1,param2,param3,param4,param5){

    var smzurl='';
    $.ajax({
        url : contPath+"/portalSer/getBjsmzUrl.do",
        async : false,
        data:{"smzUrl":param5},
        success:function(data){
            var result = eval("("+data+")");
            if(result.flag=="ok"){
                smzurl = result.smzurl;
            }else{
                layer.msg('加载数据失败!',{
                    icon : 5
                });
    ...
    if(param5 == "bjsmzUrl" && param3 == ""){
        goPage(smzurl,param1,param2,"",param4);
    }else if(param5 == "bjsmzxxUrl"){
        return smzurl;
    }else if(param5 == "bjsmzUrl" && param3 == "_blank"){
		return smzurl;
	}
}
