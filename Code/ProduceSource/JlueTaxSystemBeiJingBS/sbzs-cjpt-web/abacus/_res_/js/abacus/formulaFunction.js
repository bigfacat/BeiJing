 ...
 */
function DATE_GET_SERVER_NOW_DATE() {
...
function DATE_GET_CURRENT_DATE(DATE_FORMAT_STRING) {
	// FIXME 取服务器当前时间
	var d = DATE_GET_SERVER_NOW_DATE();
...
function VALIDATE_PHONE_NUM(num) {
	...
	if ((/^[0][0-9]{2,3}[0-9]{7,8}$/.test(num)) || (/^[0-9]{7,8}$/.test(num))
			...
			|| (/^\([0][0-9]{2,3}\)[0-9]{7,8}$/.test(num))
			|| (/^\[[0][0-9]{2,3}\][0-9]{7,8}$/.test(num))
			|| (/^[1][3,4,5,6,7,8,9][0-9]{9}$/.test(num))) {
		return true;
	} else {
		return false;
	}
...
function VALIDATE_YDPHONE_NUM(num) {
	...
	var lxdhgs = "(^[1][3,4,5,7,8,9][0-9]{9}$)";
...
function calculateMonthLastDay(nowDate) {
	...
	var month = parseInt(nowTime[1]);
	var new_year = parseInt(nowTime[0]);// 取当前的年份
 ...
 */
function fcssbYjmje() {
	var cjczbz = arguments[0];
	var jmxzdm = arguments[1];
	// 减免税房产原值或租金收入
	var jmsfcyz = arguments[2];
	// 减免税租金收入
	var jmszjsr = arguments[3];
	// var yjmje = arguments[3];

	var jmse = 0.00;

	// 如果减免性质为空，则返回原月减免金额的值，当选择了减免性质代码后才触发公式计算
	if ($.isEmptyObject(jmxzdm)) {
		return jmse;
	} else {
		var _item = this.formCT.jmxzCT[jmxzdm].item;
		var jmzlxDm = _item.jmzlxDm;
		var jmfd = _item.jmfd;
		var jmed = _item.jmed;
		var jmsl = _item.jmsl;
		...
		if (cjczbz == "cj") {
			fdsl = 0.012;
			jsbl = this.formData.fymxxx.editFymxxx.cjFymx.jsbl;
			jsyj = jmsfcyz / 1 * jsbl / 1 / 12;
			ynse = jmsfcyz / 1 * fdsl / 1 * jsbl / 1 / 12;
		} else if (cjczbz == "cz") {
			fdsl = 0.12;
			jsyj = jmszjsr;
			ynse = jmszjsr / 1 * fdsl / 1 / 12;
		}
	...
	return Math.round(jmse * 100) / 100;
}
 ...
 * @returns
 */
function checkisupperandnumberandzw(r) {
	var count = 0;
	var count1 = 0;
	var count2 = 0;
	var count3 = 0;
