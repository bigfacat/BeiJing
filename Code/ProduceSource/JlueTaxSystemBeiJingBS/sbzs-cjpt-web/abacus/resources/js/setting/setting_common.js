		...
		if(typeof validateBeforeSubmit == "function"){
			...
			if(!validateBeforeSubmit()){
				if ($("#ywbm").val().toLowerCase().indexOf("cwbb") > -1){
					rtnFlag = true;
					parent.layer.alert("数据有误或空值或报送期间与属期不匹配！",{
						icon:2,
	    				title : "错误"
	    			});
				}else {
	        ...
	        contentType:"application/json",
	 		...
	 		success:function(data){
				...
				if("Y" == jsondata.returnFlag) {
					...
					parent.layer.alert("尊敬的纳税人：G000000表中纳税人信息与当前登录纳税人信息不一致，请点击【确定】刷新页面！",{
						icon:2,
	    				title : "错误",
	    				btn1:function(index){
					...
					if(jsondata.errInfo){
						 if(jsondata.errInfo.code == "111"){
							 parent.layer.alert("财务会计制度准则与资料报送小类不匹配！",{
								icon:2,
			    				title : "错误"
							 });
			 			} else if(jsondata.errInfo.code == "101"){
			 				parent.layer.alert("数据有误或空值！",{
								icon:2,
			    				title : "错误"
			    			});
			 			} else if(jsondata.errInfo.code == "100"){
			 				parent.layer.alert("报送期间与属期不匹配！",{
								icon:2,
			    				title : "错误"
			    			});
			 			} else {
...
function getCwbbchssj(){
	...
	if(formDataHtml==null || formDataHtml=="" || formDataHtml=="undefined" || formDataHtml==undefined){
		$("body").unmask();
		parent.layer.alert("获取初始化数据失败，请稍侯再试。",{
			icon:2,
			title : "错误" 
		});
		return ;
	 		...
	 		success:function(data){
				...
				if("Y" != jsondata.returnFlag) {
					if(jsondata.errInfo){
						...
						$("body").unmask();
						parent.layer.alert("获取初始化数据失败，未知原因。",{
							icon:2,
							title : "错误"
						});
					}
