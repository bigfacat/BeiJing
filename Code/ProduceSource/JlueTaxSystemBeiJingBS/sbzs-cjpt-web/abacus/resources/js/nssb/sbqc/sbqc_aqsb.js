...
viewApp.filter('sbztDmFilter',
    function() {
        return function(item) {
            ...
            //填写按钮
//					return "<div class=\"sbtnbox\"><a class=\"sbtn sbtn01\"  href='javaScript:sbinit(\""+item.url+"\",\""+item.sbqx+"\",\""+item.skssqQ+"\",\""+item.skssqZ+"\",\"" +item.gdslxDm+"\",\""+item.zsxmDm+"\",\""+item.nsqxDm+"\",\""+item.yzpzzlDm+"\")'>填写申报表</a></div>";
                return "<a class=\"layui-btn layui-btn-sm\" style=\"height:24px;line-height:24px;\" href='javaScript:sbinit(\""+item.url+"\",\""+item.sbqx+"\",\""+item.skssqQ+"\",\""+item.skssqZ+"\",\"" +item.gdslxDm+"\",\""+item.zsxmDm+"\",\""+item.nsqxDm+"\",\""+item.yzpzzlDm+"\")'>填写申报表</a>";
        };
    });
...
viewApp.filter('cbBtnFilter',
    function() {
        return function(item) {
            ...
            //更正填写
                return "<a class=\"layui-btn layui-btn-primary\" style=\"height:24px;line-height:24px;\" href='javaScript:cbgzinit(\""+item.url+"\",\""+item.bsssqQ+"\",\""+item.bsssqZ+"\",\""+item.bsqx+"\",\""+item.bsrq+"\")'>更正财报</a>";
        };
    });
...
function showExAltr(errList) {
    ...
    //隐藏纳税人主体信息，便于定位问题
    var nsrztxxmsg = "<div><span style=\"color: #FFFFFF;white-space: normal;word-wrap: break-word;\">纳税人主体信息："+nsrztxx+"</span></div>";
    var gdssbqcZdrz = "gssbqcZdrz:" + sbqcZdrz["gssbqcZdrz"] + ",dssbqcZdrz:" + sbqcZdrz["dssbqcZdrz"];
    nsrztxxmsg += "<div><span style=\"color: #FFFFFF;white-space: normal;word-wrap: break-word;\">申报清册诊断日志："+gdssbqcZdrz+"</span></div>";
    //隐藏纳税人信息
    var msgWindow = $("#msgIframe")[0].contentWindow;// 获取子窗体中的对象
    msgWindow.addnsrmsg(nsrztxxmsg);
}
...
function showCbMsg(gdbabyzBz){
    //国地财务会计制度不一致，提示备案不一致
    if('Y'==gdbabyzBz){
        var childWindow = $("#cwbbkzIframe")[0].contentWindow;//获取子窗体中的对象
        childWindow.document.getElementById('cwbaMsgId').innerHTML = "<div style=\"padding: 5px 0; color: #FF0000;letter-spacing:1px;font-size: 12px\">温馨提示：国地税备案不一致，请修改备案！</div>";
    }
}
 ...
 */
function sbinit(url,sbqx,ssqq,ssqz,gdslxDm,zsxmDm,nsqxDm,yzpzzlDm){
    ...
    params += 'gdslxDm='+gdslxDm+'&yyyf='+yyyf+'&sbywbm=' + sbywbm+"&sbqx="+sbqx+"&yqsbbz=N";
        ...
        $.ajax({
            ...
            success : function(data) {
                ...
                if ("N" == sfkyqsbbz) {
                    ...
                    sbinit2(url,sbqx,ssqq,ssqz,gdslxDm,zsxmDm,nsqxDm,yzpzzlDm);
                }
 ...
 * 打开申报表
 */
function sbinit2(url,sbqx,skssqQ,skssqZ,gdslxDm,zsxmDm,nsqxDm,yzpzzlDm){
...
//	var sksq = parent.document.getElementById("sksq").innerText;
    ...
    var sbUrl = contextRoot+"/biz/sbqc/sbqc_aqsb/sbqxControl?type=SBQC&skssqQ="
        ...
        + "&nsqxDm="+ nsqxDm + "&tjNd="+ nd + "&tjYf=" + yf+"&yzpzzlDm="+yzpzzlDm;
    $.ajax({
        type : "post",
        async : false, // ajax同步
        url : sbUrl,
            ...
            if (qxbz == "1") {
                ...
                if(cbnbglsbnbbz=="Y" && cwbbSize>0){
                    if(sbnbqzglcbnbbz=="Y"){
                        ...
                        /*
                            // 询问提示
                            layer.confirm('尊敬的纳税人：<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;您本期还未申报财务报表，建议先进行财务报表的报送！请问是否继续申报当前申报表？', {
                                  btn: ['继续申报','退出'] // 按钮
                                }, function(){
                                      layer.msg('正在打开申报表...', {
                                            time: 100, // 0.1s后自动关闭
                            ...
                            $(".layui-layer-dialog").css("top","20%")   //询问框位置
                            */
    ...
    $.ajax({
        ...
        success : function(data) {
            ...
            if(success=="Y"){
                ...
                //展示申报表
                showSburldata(urlList,ywjkmsg,hasYwjkbz);
            }else{
 ...
 *正常展示申报表
 */
function showSburldata(urlList,ywjkmsg,hasYwjkbz){
    if(urlList.length===1){
